...
- публиковать отчеты, пригодные для анализа падений после завершения workflow

Диагностические артефакты должны покрывать как минимум:

- `test-results` со всеми вложениями Playwright по упавшим тестам
- `playwright-report` для быстрого локального и CI-анализа
- `allure-results` как сырой источник для постобработки
- `allure-report` как человекочитаемый итоговый отчет
...
В текущем состоянии репозитория файл также используется для проверки наблюдаемости падений:

- `TC-01` намеренно падает на URL-assertion для проверки стандартных failure-артефактов
- `TC-06` используется вместе с намеренно сломанным locator в page object, чтобы проверить диагностику locator-based падений
...
- локаторы описываются через `.describe(...)`, чтобы сообщения об ошибках Playwright были более читаемыми
- в текущем состоянии `searchInput` намеренно указывает на отсутствующий элемент для проверки артефактов падения по локатору
...
- консольный reporter для читаемого вывода в раннере
- HTML reporter Playwright для стандартного UI-отчета
- `allure-playwright` для генерации `allure-results`
...
- публикации диагностических отчетов
...
- сохранение `video` на падениях, если оно включено конфигурацией
- публикация CI-артефактов (`allure-report`, `playwright-report`, `test-results`)

Правило по отчетности:

- сырые результаты `allure-results` генерируются раннером во время выполнения тестов
- итоговый `allure-report` собирается отдельным шагом CI после завершения прогона
- `test-results` должен содержать вложения Playwright, необходимые для разбора конкретного падения
- в `allure-results` могут появляться markdown-вложения с контекстом ошибки, если интеграция генерирует такие файлы
- набор артефактов должен быть достаточным для диагностики без повторного запуска pipeline

Если корень проблемы неочевиден:

- первым шагом анализировать trace и контекст ошибки
- если падение связано с `locator`, отдельно смотреть его `describe(...)`, фактический селектор и timeout ожидания
